...
pub fn ticker(exchange: &str, presenter: BalancePresenter, opening_balance: Option<f64>) -> String {
    vec![
        exchange.purple().to_string(),
        format!(
            "{:.4} btc",
            presenter.total_value_in_btc()
        ),
        display_opening_balance(&presenter, opening_balance).to_string(),
        format!(
            "{} {:.2} btc {:.0}%",
            "ALTS".cyan(),
...
//            let opening_balance_usd = opening_balance_btc * btc_price;
            let profit_as_percent = price_percent(opening_balance_btc, current_balance_btc);
            // let profit_btc = current_balance_btc - opening_balance_btc;
            let profit_usd = (current_balance_btc - opening_balance_btc) * btc_price;
